-- Fix: cast all STDDEV/AVG to numeric before ROUND
...
BEGIN
  ...
  gaps AS (
    SELECT *, CASE WHEN txn_num - LAG(txn_num) OVER (PARTITION BY txn_date ORDER BY txn_num) > 3 THEN 1 ELSE 0 END AS is_new_visit
    FROM numbered
  ),
  grouped AS (
    SELECT *, SUM(is_new_visit) OVER (PARTITION BY txn_date ORDER BY txn_num) AS visit_seq
    FROM gaps
  )
  ...
  WITH visit_sizes AS (
    SELECT visit_key, COUNT(DISTINCT service_code)::numeric AS items FROM _visits GROUP BY visit_key
  ),
  co_purchase AS (
    SELECT svd.service_code,
      AVG(svd.items_per_visit) AS avg_items,
      AVG(svd.items_per_visit) - 1 AS co_rate,
  ...
  rev_trends AS (
    SELECT t.service_code,
      CASE WHEN prior90.rev > 0 THEN ROUND((recent90.rev - prior90.rev)::numeric / prior90.rev * 100, 1) END AS rev_90d,
      CASE WHEN prior30.rev > 0 THEN ROUND((recent30.rev - prior30.rev)::numeric / prior30.rev * 100, 1) END AS rev_30d
  ...
  seasonal_agg AS (
    SELECT service_code,
      MAX(CASE WHEN cnt = max_cnt THEN month_num END) AS peak_mo,
      MAX(CASE WHEN cnt = min_cnt THEN month_num END) AS low_mo,
      CASE WHEN avg_cnt > 0 THEN ROUND(CAST(STDDEV(cnt) AS numeric) / avg_cnt, 3) ELSE 0 END AS vol_volatility
    FROM (
      SELECT service_code, month_num, cnt,
        MAX(cnt) OVER (PARTITION BY service_code) AS max_cnt,
        MIN(cnt) OVER (PARTITION BY service_code) AS min_cnt,
        AVG(cnt) OVER (PARTITION BY service_code) AS avg_cnt
      FROM monthly_counts
    ) x GROUP BY service_code
  ),
  ...
  SELECT p_clinic_id, sp.service_code,
    ...
    sa.peak_mo, sa.low_mo, sa.vol_volatility,
    COALESCE(pcs.change_count, 0), pcs.avg_years,
    CASE WHEN pp.median_peer > 0 THEN ROUND((pp.current_price - pp.median_peer)::numeric / pp.median_peer * 100, 1) END,
    NULL, NULL
  FROM std_service_prices sp
  ...
  RETURN jsonb_build_object(
    'clinic_id', p_clinic_id, 'services_analyzed', computed,
