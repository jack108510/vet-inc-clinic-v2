...
BEGIN
  ...
  co_purchase AS (
    SELECT svd.service_code,
      CAST(AVG(svd.items_per_visit) AS numeric) AS avg_items,
      CAST(AVG(svd.items_per_visit) AS numeric) - 1 AS co_rate,
  ...
  rev_trends AS (
    SELECT t.service_code,
      CASE WHEN prior90.rev > 0 THEN ROUND(CAST(recent90.rev - prior90.rev AS numeric) / prior90.rev * 100, 1) END AS rev_90d,
      CASE WHEN prior30.rev > 0 THEN ROUND(CAST(recent30.rev - prior30.rev AS numeric) / prior30.rev * 100, 1) END AS rev_30d
  ...
  seasonal_agg AS (
    SELECT service_code,
      ...
      CASE WHEN AVG(cnt) > 0 THEN ROUND(CAST(STDDEV(cnt) AS numeric) / CAST(AVG(cnt) AS numeric), 3) ELSE 0 END AS vol_volatility
    FROM monthly_counts mc GROUP BY service_code
  ),
  price_change_stats AS (
    SELECT service_code, COUNT(*) AS change_count,
      CASE WHEN COUNT(*) > 1 THEN ROUND(CAST(MAX(last_seen) - MIN(first_seen) AS numeric) / 365 / COUNT(*), 2) END AS avg_years
  ...
  SELECT p_clinic_id, sp.service_code,
    ...
    CASE WHEN pp.median_peer > 0 THEN ROUND(CAST(pp.current_price - pp.median_peer AS numeric) / pp.median_peer * 100, 1) END,
    NULL, NULL
  FROM std_service_prices sp
