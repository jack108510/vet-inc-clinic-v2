...
CREATE OR REPLACE FUNCTION public.clinic_ids()
RETURNS SETOF TEXT
LANGUAGE sql
SECURITY DEFINER
STABLE
...
CREATE OR REPLACE FUNCTION public.clinic_role(check_clinic_id TEXT)
RETURNS TEXT
LANGUAGE sql
SECURITY DEFINER
STABLE
...
CREATE POLICY "users_write_own_clinic" ON std_transactions
  FOR ALL USING (
    clinic_id IN (SELECT public.clinic_ids())
    AND public.clinic_role(clinic_id) IN ('admin','superadmin')
  ) WITH CHECK (
    clinic_id IN (SELECT public.clinic_ids())
    AND public.clinic_role(clinic_id) IN ('admin','superadmin')
  );
