-- The ROUND issue: AVG() returns double precision. Cast to numeric inside CTEs.
...
BEGIN
  ...
  co_purchase AS (
    SELECT svd.service_code,
      ROUND(AVG(svd.items_per_visit)::numeric, 1) AS avg_items,
      ROUND((AVG(svd.items_per_visit) - 1)::numeric, 1) AS co_rate,
      ROUND(AVG(svd.items_per_visit)::numeric, 2) AS downstream,
  ...
  seasonal_agg AS (
    SELECT service_code,
      (SELECT month_num FROM monthly_counts mc2 WHERE mc2.service_code = mc.service_code ORDER BY cnt DESC LIMIT 1),
      (SELECT month_num FROM monthly_counts mc2 WHERE mc2.service_code = mc.service_code ORDER BY cnt ASC LIMIT 1),
      CASE WHEN AVG(cnt) > 0 THEN ROUND(STDDEV(cnt)::numeric / AVG(cnt)::numeric, 3) ELSE 0 END
    FROM monthly_counts mc GROUP BY service_code
  ),
  price_change_stats AS (
    SELECT service_code, COUNT(*),
      CASE WHEN COUNT(*) > 1 THEN ROUND((MAX(last_seen) - MIN(first_seen))::numeric / 365 / COUNT(*), 2) END
    FROM std_price_history WHERE clinic_id = p_clinic_id GROUP BY service_code
  ),
  peer_prices AS (
    SELECT sp.service_code,
      CASE WHEN PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY sp2.current_price) > 0 THEN
        ROUND((sp.current_price - PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY sp2.current_price))::numeric / PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY sp2.current_price) * 100, 1)
      END AS price_vs_peer
  ...
  SELECT p_clinic_id, sp.service_code,
    cp.top_co, cp.avg_items, cp.co_rate, cp.downstream,
    rt.rev_90d, rt.rev_30d,
    sa.col1, sa.col2, sa.col3,
    COALESCE(pcs.col1, 0), pcs.col2,
    pp.price_vs_peer,
    NULL, NULL
  FROM std_service_prices sp
