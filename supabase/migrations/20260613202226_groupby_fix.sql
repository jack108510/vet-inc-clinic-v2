...
BEGIN
  ...
  co_purchase AS (
    SELECT svd.service_code,
      AVG(svd.items_per_visit) AS avg_items, AVG(svd.items_per_visit) - 1 AS co_rate,
  ...
  seasonal_agg AS (
    SELECT service_code,
      ...
      CASE WHEN AVG(cnt) > 0 THEN ROUND(CAST(STDDEV(cnt) AS numeric) / AVG(cnt), 3) ELSE 0 END AS vol_volatility
