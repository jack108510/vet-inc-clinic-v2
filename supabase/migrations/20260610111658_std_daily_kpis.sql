-- ============================================================
-- std_daily_kpis: One row per clinic per day, pre-computed
-- Single source of truth for all dashboard KPI cards
-- ============================================================
...
CREATE TABLE IF NOT EXISTS std_daily_kpis (
  ...
  new_clients INTEGER,                 -- clients created that day
  total_visits INTEGER,                -- visits that day
  total_vaccines INTEGER,              -- vaccines administered that day
  total_appointments INTEGER,          -- appointments that day
  unique_services INTEGER,             -- distinct service_codes sold that day
...
-- Function to compute KPIs for a date range
...
BEGIN
  ...
  LEFT JOIN (
    SELECT visit_date::date AS day, COUNT(*) AS visits
    FROM std_visits
    WHERE clinic_id = p_clinic_id
  ...
  LEFT JOIN (
    SELECT vaccine_date::date AS day, COUNT(*) AS vaccines
    FROM std_vaccines
    WHERE clinic_id = p_clinic_id
      AND vaccine_date >= p_start_date
      AND vaccine_date < p_end_date
    GROUP BY vaccine_date::date
  ) vx ON vx.day = d.day
  LEFT JOIN (
    SELECT appt_date::date AS day, COUNT(*) AS appts
    FROM std_appointments
    WHERE clinic_id = p_clinic_id
      AND appt_date >= p_start_date
      AND appt_date < p_end_date
    GROUP BY appt_date::date
  ) a ON a.day = d.day
  ORDER BY d.day;
