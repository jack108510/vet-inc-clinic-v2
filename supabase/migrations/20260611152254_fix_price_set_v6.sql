...
BEGIN
  ...
  yearly_modes AS (
    SELECT service_code, EXTRACT(YEAR FROM txn_date)::int as yr, unit_price, COUNT(*) as freq
    FROM per_unit GROUP BY service_code, EXTRACT(YEAR FROM txn_date)::int, unit_price
  ),
  yearly_mode_price AS (
    SELECT DISTINCT ON (service_code, yr) service_code, yr, unit_price as mode_price
    FROM yearly_modes ORDER BY service_code, yr, freq DESC, unit_price DESC
  ),
  price_transition AS (
    SELECT DISTINCT ON (ym.service_code) ym.service_code, ym.yr as price_set_year
  ...
  SELECT 
    p_clinic_id, om.service_code, COALESCE(sv.name, om.service_code),
    om.current_price, MAKE_DATE(pt.price_set_year, 1, 1), cs.last_at_price,
