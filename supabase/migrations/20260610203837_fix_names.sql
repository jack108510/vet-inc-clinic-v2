-- Update generate_inflation_campaign to use item_names for descriptions
...
BEGIN
  ...
  inflation_factors AS (
    SELECT from_yr, EXP(SUM(LN(1 + rate))) as factor
    FROM (SELECT generate_series(2005, 2025) as from_yr) years
    CROSS JOIN cpi WHERE cpi.yr >= from_yr AND cpi.yr <= 2025
    GROUP BY from_yr
  ),
  ...
  SELECT 
    ...
    si.service_code,
    -- Prefer item_names, then std_services.name, then fall back to code
    COALESCE(iname.name, si.service_name, si.service_code),
    'inflation_catchup',
    si.current_price,
  ...
  LEFT JOIN item_names iname ON iname.code = si.service_code
  WHERE si.current_price * (si.inflation_factor - 1) > 0.50
    AND si.annual_volume * si.current_price * (si.inflation_factor - 1) > 5;

  -- Also update 99-cent campaign names
  UPDATE std_approval_queue q
  SET service_name = COALESCE(iname.name, q.service_name)
  FROM item_names iname
  WHERE q.clinic_id = p_clinic_id
    AND q.strategy = 'price_ending'
    AND q.service_code = iname.code;
