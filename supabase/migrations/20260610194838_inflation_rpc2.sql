-- Generate inflation catchup recommendations
...
BEGIN
  ...
  SELECT 
    ...
    ROUND((i.suggested_price - i.current_price) / NULLIF(i.current_price, 0) * 100, 1),
    ROUND(i.annual_volume * (i.suggested_price - i.current_price) / 12, 2),
    ROUND(i.annual_volume * (i.suggested_price - i.current_price), 2),
    CASE 
      WHEN (i.suggested_price - i.current_price) / NULLIF(i.current_price, 0) > 0.15 THEN 'high'
      WHEN (i.suggested_price - i.current_price) / NULLIF(i.current_price, 0) > 0.08 THEN 'medium'
      ELSE 'low'
    END,
    'pending',
    CASE 
      WHEN (i.suggested_price - i.current_price) / NULLIF(i.current_price, 0) > 0.15 THEN 'urgent'
      WHEN (i.suggested_price - i.current_price) / NULLIF(i.current_price, 0) > 0.08 THEN 'high'
      ELSE 'normal'
    END
  ...
  SELECT json_build_object(
    'count', COUNT(*),
    'total_uplift', SUM(expected_annual_uplift)
  ) INTO result
