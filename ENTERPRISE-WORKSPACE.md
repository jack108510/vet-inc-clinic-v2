...
- Enterprise tables are intentionally separate from the legacy clinic pricing tables, because existing legacy `price_approvals` and `price_recommendations` have anonymous policies. Do not migrate customer records or use those legacy tables for enterprise sign-off without a separate security review.
